...
const cityOptions = ["All cities", ...supportedCities];
...
export function BrowseCompanionsPage({ initialCity }: { initialCity?: string } = {}) {
  const { member } = useSessionMember();
  const [directory, setDirectory] = useState<DirectoryResponse | null>(null);
  const [loading, setLoading] = useState(true);
  const [city, setCity] = useState(initialCity && cityOptions.includes(initialCity) ? initialCity : "All cities");
  ...
  useEffect(() => {
    const requestedCity = initialCity || new URLSearchParams(window.location.search).get("city");
  ...
  return <main className="min-h-screen overflow-hidden bg-[#0d0918] text-white">
    ...
    <section ref={filtersRef} aria-label="Companion directory filters" className="sticky top-0 z-30 border-y border-white/10 bg-[#0d0918]/95 px-5 py-3.5 backdrop-blur-xl lg:px-8"><div className="mx-auto flex max-w-7xl flex-wrap items-center gap-2.5"><div className="mr-1 inline-flex h-10 items-center gap-2 rounded-xl border border-cyan-200/20 bg-cyan-200/[.06] px-3 text-sm font-bold text-cyan-100"><Filter size={16} /> <span className="hidden sm:inline">Filters</span>{activeFilterCount > 0 && <span className="inline-flex h-5 min-w-5 items-center justify-center rounded-full bg-cyan-200 px-1 text-xs leading-none text-mauve-950">{activeFilterCount}</span>}</div><FilterSelect label="Vibe" value={vibe} options={vibeOptions.map((item) => ({ label: item, value: item }))} onChange={(value) => { setVibe(value); setVisibleCount(12); }} /><FilterSelect label="Age range" value={String(ageRangeIndex)} options={ageRanges.map(([label], index) => ({ label, value: String(index) }))} onChange={(value) => { setAgeRangeIndex(Number(value)); setVisibleCount(12); }} /><FilterSelect label="Availability" value={availability} options={availabilityOptions.map(([label, value]) => ({ label, value }))} onChange={(value) => { setAvailability(value as Availability | "all"); setVisibleCount(12); }} /><FilterSelect label="Language" value={language} options={languageOptions.map((item) => ({ label: item, value: item }))} onChange={(value) => { setLanguage(value); setVisibleCount(12); }} /><FilterSelect label="Sort by" value={sort} options={[{ label: "Recommended", value: "recommended" }, { label: "Newest", value: "newest" }, { label: "Most popular", value: "popular" }]} onChange={setSort} /><button type="button" onClick={() => { if (isPrismMember) { setPrismOnly((value) => !value); setVisibleCount(12); } }} aria-pressed={prismOnly} className={cn("inline-flex h-10 items-center gap-1.5 rounded-xl border px-3.5 text-sm font-bold transition", prismOnly ? "border-amber-100 bg-amber-100 text-mauve-950" : "border-amber-100/30 bg-amber-100/[.06] text-amber-100 hover:bg-amber-100/10", !isPrismMember && "cursor-pointer")} title={isPrismMember ? "Show PRISM-exclusive companions" : "PRISM unlocks exclusive companions"}><Sparkles size={14} /> PRISM Only</button>{activeFilterCount > 0 && <button type="button" onClick={clearFilters} className="h-10 rounded-xl px-3 text-sm font-semibold text-violet-100/65 transition hover:bg-white/[.06] hover:text-cyan-100">Clear filters</button>}</div></section>
