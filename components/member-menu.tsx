    ...
    <AnimatePresence>
      {open && <motion.div initial={{ opacity: 0, y: -8, scale: .96 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -8, scale: .96 }} transition={{ duration: .18 }} role="dialog" className="absolute right-0 top-12 z-[70] w-[min(18rem,calc(100vw-2rem))] max-h-[calc(100vh-5rem)] overflow-y-auto rounded-2xl border border-mauve-200 bg-mauve-50 p-3 shadow-2xl [scrollbar-width:thin] dark:border-mauve-700 dark:bg-mauve-900">
        ...
        <div className="mt-2 grid gap-1">
          ...
          <Link onClick={() => setOpen(false)} href="/messages" className="flex items-center gap-2 rounded-xl px-3 py-2.5 text-sm font-medium transition hover:bg-mauve-100 dark:hover:bg-mauve-800"><MessageCircle size={15} /> {t("common", "messages")}</Link>
