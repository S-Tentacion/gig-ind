...
export function FilterSelect({ label, value, options, onChange }: { label: string; value: string; options: Option[]; onChange: (value: string) => void }) {
  ...
  const selected = tr(options.find((option) => option.value === value)?.label ?? value);
  ...
  return <div data-no-translate className="relative" ref={container}>
    ...
    {open && <div role="listbox" aria-label={tr(label)} className="absolute left-0 top-[calc(100%+.5rem)] z-[70] min-w-full overflow-hidden rounded-xl border border-cyan-100/20 bg-[#171124] p-1.5 shadow-2xl shadow-black/50"><p className="px-2.5 py-2 text-[10px] font-bold uppercase tracking-[.16em] text-violet-100/45">{tr(label)}</p>{options.map((option) => <button key={option.value} role="option" aria-selected={option.value === value} type="button" onClick={() => { onChange(option.value); setOpen(false); }} className={`flex w-full items-center justify-between gap-4 rounded-lg px-2.5 py-2.5 text-left text-sm transition ${option.value === value ? "bg-cyan-200 text-mauve-950" : "text-violet-100/80 hover:bg-white/[.07] hover:text-white"}`}><span>{tr(option.label)}</span>{option.value === value && <Check size={15} />}</button>)}</div>}
  </div>;
}
