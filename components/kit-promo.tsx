...
export function KitPromo({ name }: { name: string }) {
  ...
  const words = ["PRISM MEMBER HOME", "GIGOLO KIT", "PRIVATE MEMBER LOOK", "COINGATE CHECKOUT"];
  return <section className="relative mx-auto max-w-7xl overflow-hidden px-5 pt-2 lg:px-8">
    <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .6 }} className="relative overflow-hidden rounded-[2rem] border border-fuchsia-300/50 bg-mauve-950 px-6 py-8 text-mauve-50 shadow-2xl sm:px-10 sm:py-10">
      ...
      <div className="relative grid min-w-0 gap-7 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center"><div className="min-w-0"><p className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[.17em] text-cyan-200"><Sparkles size={14}/> A brighter layer for {name}</p><h2 className="mt-3 max-w-2xl font-serif text-4xl tracking-tight sm:text-5xl">Step into your PRISM experience.</h2><p className="mt-3 max-w-xl text-sm leading-6 text-mauve-200">Gigolo Kit unlocks a private PRISM member home, a distinct identity, and an elevated experience designed around discretion.</p></div><Link href={ROUTES.membership} className="button-shine inline-flex w-fit items-center justify-center gap-2 rounded-full bg-mauve-50 px-5 py-3.5 text-sm font-semibold text-mauve-950 shadow-lg transition hover:scale-[1.02] lg:justify-self-end"><Crown size={16}/> Explore PRISM membership <ArrowRight size={16}/></Link></div>
