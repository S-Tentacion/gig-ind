...
function PrismUpgradeModal({ open, source, member, onClose }: { open: boolean; source: string; member: SessionMember | null; onClose: () => void }) {
  ...
  const completePayment = async () => {
    ...
    try {
      const order = await createTelegramOrder({ kind: "kit", source }, popup);
  ...
  return <div role="presentation" onMouseDown={onClose} className="fixed inset-0 z-[100] flex items-end bg-[#090611]/80 p-0 backdrop-blur-md sm:items-center sm:justify-center sm:p-5">
    <div ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="prism-upgrade-title" onMouseDown={(event) => event.stopPropagation()} className="max-h-[94dvh] w-full overflow-y-auto rounded-t-[2rem] border border-amber-100/30 bg-[radial-gradient(circle_at_10%_0%,rgba(251,191,36,.18),transparent_30%),radial-gradient(circle_at_100%_100%,rgba(34,211,238,.14),transparent_32%),#171021] p-5 shadow-2xl shadow-black/60 sm:max-w-[30rem] sm:rounded-[2rem] sm:p-7">
      ...
      {state === "default" && <><div className="mt-6 rounded-2xl border border-amber-100/25 bg-amber-100/[.07] px-5 py-4"><p className="font-serif text-4xl text-amber-100">⭐ 8,750</p><p className="mt-1 text-xs font-medium text-violet-100/65">One-time Telegram Stars payment · Lifetime PRISM access</p></div><div className="mt-6 grid gap-3 sm:grid-cols-2">{benefits.map(([Icon, title, text]) => <div key={title} className="rounded-xl border border-white/10 bg-white/[.035] p-3"><Icon size={16} className="text-amber-100" /><p className="mt-2 text-xs font-bold text-white">{title}</p><p className="mt-1 text-[11px] leading-4 text-violet-100/60">{text}</p></div>)}</div><div className="mt-6 border-t border-white/10 pt-5"><button type="button" onClick={() => void completePayment()} className="inline-flex w-full items-center justify-center gap-2 rounded-full bg-gradient-to-r from-amber-100 via-[#f3cf85] to-cyan-100 px-5 py-3.5 text-sm font-bold text-mauve-950 transition hover:brightness-110"><Crown size={17} /> Pay ⭐ 8,750 in Telegram</button><button type="button" onClick={onClose} className="mt-3 w-full text-sm font-semibold text-violet-100/65 transition hover:text-white">Maybe later</button><p className="mt-4 flex items-center justify-center gap-1.5 text-center text-[11px] text-violet-100/60"><LockKeyhole size={12} /> Secure Telegram Stars payment</p><p className="mt-2 text-center text-[10px] leading-4 text-violet-100/45">Payments are non-refundable except as required by law. See Terms for details.</p></div></>}
      {state === "loading" && <div className="grid min-h-72 place-items-center text-center"><div><LoaderCircle size={34} className="mx-auto animate-spin text-amber-100" /><p className="mt-5 text-sm font-semibold text-white">Waiting for Telegram payment…</p></div></div>}
