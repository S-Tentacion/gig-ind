...
import { ROUTES, type AppLocale } from "@/lib/routes";

const geist = Geist({ subsets: ["latin"], variable: "--font-sans" });
const siteUrl = (process.env.NEXT_PUBLIC_SITE_URL || "https://gig-ind.vercel.app").replace(/\/$/, "");
const privatePaths = new Set<string>([ROUTES.login, ROUTES.register, ROUTES.setPassword, ROUTES.profile, ROUTES.kitOrders, ROUTES.boostCredits, ROUTES.payments, ROUTES.messages, ROUTES.buy, ROUTES.join, ROUTES.authCallback]);

const pageDetails: Record<string, { title: string; description: string }> = {
  [ROUTES.home]: { title: "Gigolo India | Verified Male Companions & Escorts Across India", description: "Meet verified, discreet male companions across India. Browse profiles, chat privately, and book safely. Adults 18+ only." },
  [ROUTES.browse]: { title: "Browse Verified Companions | Gigolo India", description: "Browse private previews of verified adult companion profiles across India. Sign in for member access." },
  [ROUTES.membership]: { title: "PRISM Premium Membership | Gigolo India", description: "Unlock lifetime PRISM access with unblurred galleries, priority booking, exclusive companions, concierge service, and private member benefits." },
  [ROUTES.becomeCompanion]: { title: "Become a Verified Companion | Gigolo India", description: "Learn about the companion application, verification, safety, and privacy process at Gigolo India." },
  [ROUTES.privacy]: { title: "Privacy Policy | Gigolo India", description: "Read Gigolo India's privacy, eligibility, safety, and consent-first community policy." },
  [ROUTES.terms]: { title: "Terms of Use | Gigolo India", description: "Read the terms for using Gigolo India safely, respectfully, and in accordance with local laws." },
  [ROUTES.safety]: { title: "Safety Standards | Gigolo India", description: "Practical safety, consent, privacy, and reporting standards for Gigolo India members." },
  [ROUTES.communityGuidelines]: { title: "Community Guidelines | Gigolo India", description: "The consent, privacy, respect, and safety standards for the Gigolo India community." },
  [ROUTES.howItWorks]: { title: "How Gigolo India Works | Verified Adult Member Access", description: "Learn how verified adult members browse profiles, chat privately, set clear boundaries, and use Gigolo India safely." },
  [ROUTES.faq]: { title: "Frequently Asked Questions | Gigolo India", description: "Answers about membership, verification, privacy, safety, and using Gigolo India responsibly." },
};
...
function getRequestContext(headersList: Headers) {
  ...
  const locale: AppLocale = match?.[1] === "hi" ? "hi" : "en";
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const detail = cityName ? { title: `Male Companions in ${cityName} | Gigolo India`, description: `Browse verified adult male companion profiles in ${cityName}. Private member access, clear boundaries, and safety-first guidance.` } : pageDetails[context.pagePath] || pageDetails[ROUTES.home];
